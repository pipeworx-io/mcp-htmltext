...
`tools/list` at `https://gateway.pipeworx.io/htmltext/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/html_to_text \
  -H 'Content-Type: application/json' \
  -d '{"html":"<html><body><h1>Welcome</h1><p>This is a <strong>test</strong> page.</p><script>console.log('\''hidden'\'');</script></body></html>"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/html_to_text`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
